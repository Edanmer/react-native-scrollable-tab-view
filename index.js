...
const ScrollableTabView = createReactClass({
    ...
    if (Platform.OS === "ios") {
      ...
      if (this.scrollView) {
        ...
        if (this.props.scrollWithoutAnimation) {
          this.scrollView._component.setPageWithoutAnimation(pageNumber);
        } else {
          this.scrollView._component.setPage(pageNumber);
        }
      }
    }
  ...
  _composeScenes() {
    return this._children().map((child, idx) => {
      let key = this._makeSceneKey(child, idx);
      return (
        <SceneComponent key={child.key} shouldUpdated={this._shouldRenderSceneKey(idx, this.state.currentPage)} style={{ width: this.state.containerWidth }}>
